...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiResponse> chatExceptionHandler(ChatException chatException, WebRequest webRequest) {
		logger.error("ChatException: {}", webRequest.getDescription(true), chatException);
		ApiResponse apiErrorResponse = new ApiResponse(chatException.getMessage(),
				false);
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(apiErrorResponse);
	}

	@ExceptionHandler(MessageException.class)
	public ResponseEntity<ApiResponse> messageExceptionHandler(MessageException messageException,
			WebRequest webRequest) {
		logger.error("MessageException: {}", webRequest.getDescription(true), messageException);
		ApiResponse apiErrorResponse = new ApiResponse(messageException.getMessage(),
				false);
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(apiErrorResponse);
	}
